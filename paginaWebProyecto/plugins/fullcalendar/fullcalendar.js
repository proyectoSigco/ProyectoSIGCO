...
var Popover = Class.extend({
	...
	// Triggered when the iterator clicks *anywhere* in the document, for the autoHide feature
...
var DragListener = Class.extend({
	...
	// Call this when the iterator does a mousedown. Will probably lead to startListening
	...
	// Called when the iterator moves the mouse
	...
	// Called when the iterator does a mouseup
	mouseup: function(ev) {
		this.stopDrag(ev);
	...
	// Call this to stop listening to the iterator's mouse events
...
var MouseFollower = Class.extend({
	...
	// Gets called when the iterator moves the mouse
	mousemove: function(ev) {
...
Grid.mixin({
	...
	mousedOverSeg: null, // the segment object the iterator's mouse is over. null if over nothing
	...
	destroyEvents: function() {
		this.triggerSegMouseout(); // trigger an eventMouseout if iterator's mouse is over an event
	...
	// Called when the iterator does a mousedown on an event, which might lead to dragging.
	...
	// Called when the iterator does a mousedown on an event's resizer, which might lead to resizing.
	// Generic enough to work with any type of Grid.
...
DayGrid.mixin({
	...
	showSegPopover: function(cell, moreLink, segs) {
		...
		options = {
			...
			autoHide: true, // when the iterator clicks elsewhere, hide the popover
...
var View = fc.View = Class.extend({
	...
	isSelected: false, // boolean whether a range of time is iterator-selected or not
	...
	// Computes the new date when the iterator hits the prev button, given the current date
	...
	// Computes the new date when the iterator hits the next button, given the current date
	...
	// Sets the scroll value of the scroller to the initial pre-configured state prior to allowing the iterator to change it
	...
	// Computes if the given event is allowed to be dragged by the iterator
	...
	// Computes if the given event is allowed to be resize by the iterator
	...
	// Handler for unselecting when the iterator clicks something and the 'unselectAuto' setting is on
	documentMousedown: function(ev) {
		var ignore;

		// is there a selection, and has the iterator made a proper left click?
	...
	function refetchEvents() { // can be called as an API method
		destroyEvents(); // so that events are cleared before iterator starts waiting for AJAX
